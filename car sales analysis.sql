...
-- analyse the power produced by the engine of each car model
...
-- replace the model price with 0 with the actual price

-- first find the model name with 0 price allocated.
...
SELECT m.manufacturer_name, SUM(s.sales) AS revenue_generated
FROM sales s
LEFT JOIN manufacturer m
ON s.manufacturer_id = m.manufacturer_id
GROUP BY m.manufacturer_name
